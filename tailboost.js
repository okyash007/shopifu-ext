...
export async function createApp(shop, accessToken) {
  ...
  const createdAppData = await fetch(
    `https://staging-tintin.tailboost.ai/createApp?app_name=Tailboost&secret_key=${app.api_secret}&api_key=${app.api_key}&tailboost_app_name=${app.app_name}&tailboost_app_store_url=${app.store_url}&tailboost_app_store_email=${store.email}&callback_url=https://dummyurl&redirect_url=https://dummyurl`,
    {
      method: "GET",
      headers: {
...
export async function logUser(data) {
  const response = await fetch(`https://staging-tintin.tailboost.ai/loguser`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      Accept: "application/json",
